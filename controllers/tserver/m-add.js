                            ...
                            ssh.execCommand('curl -O https://optimuscp.io/bash/os.sh && chmod +x os.sh && ./os.sh && rm -f os.sh')
                                .then(function(result) {
                                    ...
                                    if ((result.stdout.split('\n')[0].indexOf("CentOS") >= 0) || (result.stdout.split('\n')[0].indexOf("Ubuntu") >= 0)) {
                                        ...
                                        ssh.execCommand('sudo useradd -m -ou 0 -g 0 -s /bin/bash optimusCP && echo -e "' + team.added[team.added.length - 1]._id + '\n' + team.added[team.added.length - 1]._id + '" | passwd optimusCP && echo "optimusCP ALL=(ALL:ALL) NOPASSWD: ALL" >> /etc/sudoers && echo "AllowUsers optimusCP\@optimuscp.io" >> /etc/ssh/sshd_config && service ssh restart')
                                            .then(function(result) {
                                                if (!result.stderr.includes('already exists')) {
                                                    ssh.connect({
                                                        ...
                                                        .then(function() {
                                                            ssh.execCommand('sudo ' + oscmd + ' -y install wget && wget -O metrics.sh https://optimuscp.io/bash/tmetrics.sh && chmod +x metrics.sh && sed -i "s/\x0D$//" metrics.sh && (crontab -l ; echo "*/5 * * * * /home/optimusCP/metrics.sh ' + team._id + ' ' + team.added[team.added.length - 1]._id + '") 2>&1 | grep -v "no crontab" | sort | uniq | crontab -')
                                                                .then(function(result) {
                                                                    team.save();
                        ...
                        if (!result.stderr) {
                            ssh.execCommand('curl -O https://optimuscp.io/bash/os.sh && chmod +x os.sh && ./os.sh && rm -f os.sh')
                                .then(function(result) {
                                    ...
                                    if ((result.stdout.split('\n')[0].indexOf("CentOS") >= 0) || (result.stdout.split('\n')[0].indexOf("Ubuntu") >= 0)) {
                                        var oscmd = (result.stdout.split('\n')[0].indexOf("CentOS") >= 0) ? 'yum' : 'apt-get';
                                        ssh.execCommand('sudo -i /bin/bash -c "sudo useradd -m -ou 0 -g 0 -s /bin/bash optimusCP && echo -e \\"' + user.added[user.added.length - 1]._id + '\\n' + user.added[user.added.length - 1]._id + '\\" | sudo passwd optimusCP && echo \\"optimusCP ALL=(ALL:ALL) NOPASSWD: ALL\\" >> /etc/sudoers && service ssh restart && sudo sed -i \\"s/^PasswordAuthentication.*/PasswordAuthentication yes/\\" /etc/ssh/sshd_config && sudo sed -i \\"s/^PermitRootLogin.*/PermitRootLogin yes/\\" /etc/ssh/sshd_config && echo "AllowUsers optimusCP\@optimuscp.io" >> /etc/ssh/sshd_config && service sshd reload"')
                                            .then(function(result) {
